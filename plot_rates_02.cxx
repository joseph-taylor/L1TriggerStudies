...
class rootPlotMaker{
	...
	//private...only seen by class functions...
	// int width, height;

public:
	//can be accessed anywhere in the main script...



	//got to be careful i think...
	TFile * f1 = 0;
	//TFile * f2 = 0;
	...
	//multiple 1d plots
	vector<TH1F*> histogramList = 0;




	void loadFile(string inputFileName){

		f1 = TFile::Open( inputFileName.c_str() );

		int testingInteger = 7;

		return;
	}


//not sure about histodummy...got to use objects from within the class?
	void loadHistogram(string histoName1){
		h1 = (TH1F*)f1->Get( histoName1.c_str() );
		return;
	}



	void drawHistogram(TH1F * hdummy){
...
//defining the function seperately
void rootPlotMaker::set_values(int x, int y){
...
void trial(){
	...
	// TFile * F = plot1.loadFile("output_rates/run269224_zeroBias_v34p0/histos.root");
    // TH1F * h1 = (TH1F*)F->Get( "metSumDist_emu" );
    // h1->Draw();


	plot1.loadFile("output_rates/run269224_zeroBias_v34p0/histos.root");

	cout << plot1.testingInteger << endl << endl;

	plot1.loadHistogram("mhtSumDist_emu");
	plot1.drawHistogram(plot1.h1);




    // vector<string> histostringname;
    // histostringname.push_back("mhtSumDist_emu");
    // histostringname.push_back("htSumDist_hw");
    // histostringname.push_back("metSumDist_hw");    

    // cout << "testing...";
    // cout << histostringname[1];


    // TH1F * h1 = (TH1F*)F->Get(histostringname[1].c_str());
    // h1->Draw();
...
//one output file names, one output directory, one xaxisname, one yaxisname
//x set range, ysetrange, 
// log, legend, legend coords,
// all that formatting stuff
//additional text and auto position


//vectors

//legend1 nam, legend2 nam, legend3 nam, legend4 nam...
